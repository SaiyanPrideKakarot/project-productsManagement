...
const { uploadProfileImages } = require('../controllers/awsController')
...
const createUser = async function (req, res) {
    try {
        ...
        let uploadedImageUrl = await uploadProfileImages(profileImage[0])
...
const getUserProfile = async function (req, res) {
    try {
        ...
        if (!userProfile) {
            return res.status(404).send({ status: false, message: "User doesnot exists" });
        }
...
const updateAPI = async function (req, res) {
    try {
        ...
        let bodylength = Object.keys(data).length
        if (bodylength == 0 || bodylength > 7) {
            return res.status(400).send({ status: false, message: "Invalid request body" })
        }
        if (fname || lname || email || phone || password || profileImage || address) {
            let obj = {};
            if (fname) {
                fname = fname.trim()
                if (!isValidName(fname)) {
            ...
            if (profileImage) {
                ...
                let uploadedImageUrl = await uploadProfileImages(file[0])
